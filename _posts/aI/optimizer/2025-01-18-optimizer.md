...
### Adam (Adaptive Moment Estimation)
  ...
  장점  
  + 대부분 딥러닝 문제에서 안정적이고 빠르게 수렴
  + 학습률을 자동으로 조정
  + 희소 데이터와 대규모 데이터 모두에 적합
  + SGD에 비해 과적합 가능성이 낮고 수렴 속도가 빠름  
  단점  
  + 학습률 초기 설정이 여전히 결과에 영향을 줄 수 있음
  + 일부 문제에서는 과적합을 초래할 가능성이 있음
  + 모멘텀 매개변수 설정이 복잡한 문제에서는 민감할 수 있음

### Nadam (Nesterov-accelerated Adaptive Moment Estimation)
  Adam에 Nesterov Momentum 추가, 기울기 계산 전 미리 한 단계 앞으로 이동하여 기울기의 변화를 더 잘 반영   
  장점  
  + Adam보다 더 빠르게 수렴하며 안정적.
  + Nesterov Momentum 덕분에 기울기 방향을 더 정확히 예측.
  + 대부분의 데이터셋에서 높은 성능과 빠른 학습 속도 제공
  + Adam의 장점을 계승하면서도 더욱 효과적  
  단점  
  + 계산량이 Adam보다 약간 더 많음
  + Nadam이 항상 Adam보다 좋은 것은 아니다
  + 초기 학습률 설정이 여전히 중요하다
