...
## Parametric Estimation (모수 추정)
  모집단의 분포 형태를 가정하고, 해당 분포의 모수(평균, 분산 등)를 추정하는 방법이다.  
  모집단의 분포가 정규분포, 이항분포 등 특정 분포를 따른다고 가정하고 추정한다.  
  모집단을 있다고 가정하는 추정이며, 주로 쓰인다.


## Non-Parametric Estimation (비모수 추정)
  모집단의 분포 형태를 가정하지 않고, 데이터를 기반으로 직접 추정하는 방법이다.  
  모집단의 분포를 가정하지 않고, 데이터에 의존한다.  
  표본을 모집단으로 본다고 할 수 있다.  


## Central limit theorem (중심 극한 정리)
  표본의 크기가 충분히 크다면, 표본 평균의 분포는 모집단의 분포 형태와 관계없이 정규분포에 가까워진다.  
  조건  
  1. 표본의 크기가 30 이상이어야 한다.
  2. 독립적 표본이어야 한다.
  3. 모집단의 분산이 유한해야 한다.


## i.i.d. (Independent Identically Distribution) 독립 동일 분포
  통계학과 확률론에서 데이터의 특성을 나타내는 중요한 가정이다.  
  이 조건에 만족하지 않으면 데이터 학습에 아무런 의미가 없다.  
  조건  
  1. Independence (독립성)  
   - $ P(X_1, X_2, \dots, X_n ) = P(X_1)\cdot P(X_2) \cdot \dots \cdot P(X_n) $  
   - 확률변수들이 서로 독립적이다.  
   - 한 변수의 값이 다른 변수의 값에 영향을 미치지 않는다.  
  2. Identical Distribution (동일분포)  
   - $ F_{X_1}(x) = F_{X_2}(x) = \cdots = F_{X_n}(x) $  
     * $ F_{X_i}(x) $: 확률변수 $X_i$의 누적분포함수(CDF)  
   - 모든 확률변수들이 동일한 확률분포를 따른다.  
